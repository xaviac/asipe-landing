...
// --- Brand assets & palette (replace paths and colors with your project files) ---
const brand = {
  logoPrimary: "/assets/ASIPE.jpg", // ← pon aquí tu logo horizontal
  logoMark: "/assets/asipe-mark.svg", // ← y aquí tu isotipo/imagotipo
  colors: {
    primary: "#0E4CFF", // ← sustituye por tu color principal
    primaryDark: "#0B3CC9", // ← variante oscura del primario
    accent: "#7C3AED", // ← color acento (degradados / highlights)
    neutral: "#0F172A", // ← texto principal
    light: "#EEF2FF", // ← fondos suaves
  },
} as const;
...
const inputBase =
  "block w-full rounded-xl border border-gray-300 bg-white/70 px-4 py-3 text-gray-900 placeholder-gray-500 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200" as const;

type LeadForm = { nombre: string; email: string; telefono: string; mensaje: string };

export default function Site() {
  const [form, setForm] = useState<LeadForm>({ nombre: "", email: "", telefono: "", mensaje: "" });
...
if (typeof window !== "undefined") {
  try {
    ...
    console.assert(typeof (Site as any) === "function", "Site debe exportar un componente por defecto");

    // Additional tests
    console.assert(Array.isArray((services as any)) && services.length > 0, "services debe contener al menos un servicio");
